...
- `services/api/Api.sln`: Control Plane API (user/app/build/deployment management)
...
- `services/web`: Frontend (React, Vite, Tailwind)
- `services/webapp`: Data Plane (Gateway, SPA worker, Go deployment worker)
- `deploy/`: Terraform (infra), Ansible (deploy), Docker Compose (prod config)

## Developer Workflows
- **Build API**: `dotnet build services/api/Api.sln`
- **Run API**: `dotnet run --project services/api/Api`
- **Run Gateway**: `dotnet run --project services/webapp/gateway`
...
- **Frontend**: Uses Vite, Tailwind, TypeScript. See `services/web/` for conventions.
...
- To add a new frontend page: add to `services/web/src/pages/`, update routes in `services/web/src/App.tsx`.
- To add a new secret: define in `infra/aws_secrets.tf`, reference in `pkg/`, and list in `scripts/deploy.yml`.

## References
- See `services/api/AGENTS.md` and `CLAUDE.md` for more details on architecture and workflows.
