...
- `src/pkg/api/Api.sln`: Control Plane API (user/app/build/deployment management)
...
- `src/pkg/web`: Frontend (React, Vite, Tailwind)
- `src/pkg/webapp`: Data Plane (Gateway, SPA worker, Go deployment worker)
- `deploy/`: Terraform (infra), Ansible (deploy), Docker Compose (prod config)

## Developer Workflows
- **Build API**: `dotnet build src/pkg/api/Api.sln`
- **Run API**: `dotnet run --project src/pkg/api/Api`
- **Run Gateway**: `dotnet run --project src/pkg/webapp/gateway`
- **Frontend Dev**: `npm run dev --prefix src/pkg/web`
- **Frontend Build**: `npm run build --prefix src/pkg/web`
- **Lint**: `npm run lint --prefix src/pkg/web`
- **Go Worker**: `cd src/pkg/webapp/spa/worker && go build ./...`
- **Migrations**: `dotnet ef migrations add <Name> --project src/pkg/api/Api.Migrations`
- **Integration Tests**: `dotnet test src/pkg/api/Api.MigrationTest`
...
- **Frontend**: Uses Vite, Tailwind, TypeScript. See `src/pkg/web/` for conventions.
...
- To add a new frontend page: add to `src/pkg/web/src/pages/`, update routes in `src/pkg/web/src/App.tsx`.
- To add a new secret: define in `infra/aws_secrets.tf`, reference in `pkg/`, and list in `scripts/deploy.yml`.

## References
- See `src/pkg/api/AGENTS.md` and `CLAUDE.md` for more details on architecture and workflows.
